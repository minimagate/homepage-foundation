...
The shared source is `homepage-foundation/core.css`. Each Zola site imports it from its `static/styles/foundation` submodule before its local stylesheet rules. Read [the design-system reference](references/design-system.md) before a visual change that affects hierarchy, spacing, type, links, icons, or responsive layout.
...
Favor shared selectors and aliases already present in the core. Do not create a site-specific exception merely to rename an otherwise common component. The family is intentionally text-first and quiet: changing it toward cards, decorative hero treatment, heavy borders, large display type, or colour accents needs an explicit design request.
