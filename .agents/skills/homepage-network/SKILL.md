...
Use the focused skill as well when the request falls into one of these paths: `shared-homepage-styles` for the common visual system, `homepage-editorial` for writing, `homepage-zola-templates` for shell/partial work, `homepage-research-publication` for research publication, and `homepage-bivouac-publication` for Bivouac work. This skill decides ownership and coordination; it does not replace the destination repository's `AGENTS.md`.
